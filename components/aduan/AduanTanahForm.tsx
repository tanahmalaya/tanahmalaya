...
export default function AduanTanahForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!anonim && benarkanDihubungi && (!namaPenuh.trim() || !telefon.trim())) {
      setRalat("Sila lengkapkan nama & no. telefon untuk dihubungi, atau pilih kekal anonim.");
      return;
    }
    if (!pengesahanMaklumat || !persetujuanPdpa) {
      setRalat("Sila tandakan kedua-dua kotak pengesahan & persetujuan PDPA di Bahagian 5.");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      {/* Bahagian 1 */}
      <fieldset className="bg-white rounded-md shadow-sm p-5 space-y-4">
        <legend className="font-display font-bold px-1">Bahagian 1: Maklumat Pengadu</legend>
      ...
      {/* Bahagian 2 */}
      <fieldset className="bg-white rounded-md shadow-sm p-5 space-y-4">
        <legend className="font-display font-bold px-1">Bahagian 2: Lokasi dan Identiti Tanah</legend>
        <p className="text-xs text-brand-dark/60">
      ...
      {/* Bahagian 3 */}
      <fieldset className="bg-white rounded-md shadow-sm p-5 space-y-4">
        <legend className="font-display font-bold px-1">Bahagian 3: Butiran Isu &amp; Pencerobohan</legend>
      ...
      {/* Bahagian 4 */}
      <fieldset className="bg-white rounded-md shadow-sm p-5 space-y-5">
        <legend className="font-display font-bold px-1">Bahagian 4: Bukti Fizikal &amp; Lampiran</legend>
      ...
      {/* Bahagian 5 */}
      <fieldset className="bg-white rounded-md shadow-sm p-5 space-y-3">
        <legend className="font-display font-bold px-1">Bahagian 5: Pengesahan &amp; Penafian</legend>
        ...
        <label className="flex items-start gap-2 text-sm">
          <input
            ...
            className="mt-0.5"
          />
          Saya membenarkan Pertubuhan Literasi Tanah memproses maklumat ini untuk tujuan rekod, kajian,
          dan rujukan tindakan undang-undang/badan berkuasa.
        </label>
        ...
        <p className="text-xs text-brand-dark/50 pt-2 border-t border-brand-cream">
          ...
          tempatan (PBT), atau Polis Diraja Malaysia (PDRM). Pihak Pertubuhan tidak bertanggungjawab
          secara langsung atas sebarang implikasi perundangan antara pihak-pihak yang terlibat.
        </p>
      </fieldset>
